...
# Function to make predictions and update sentence
def make_prediction_and_update_sentence(sequence, model, actions, threshold, sentence):
    res = model.predict(np.expand_dims(sequence, axis=0))[0]
    prediction_index = np.argmax(res)
    print(actions[prediction_index])

    if np.max(res) > threshold:
        if len(sentence) > 0:
            if actions[prediction_index] != sentence[-1]:
                sentence.append(actions[prediction_index])
        else:
            sentence.append(actions[prediction_index])
